...
objectsThatMoved = objects(objects.d1 > 0, :);

for rowIdx = 1:height(objectsThatMoved)
    object = objectsThatMoved(rowIdx, :);
    fprintf("The %s needs to be pushed % .2f m\n", object.name, object.d1);
end

objectsThatDidNotMove = objects(objects.d1 <= 0, :);

if(height(objectsThatDidNotMove) > 0)
    fprintf("\n")
end

for rowIdx = 1:height(objectsThatDidNotMove)
    fprintf("The %s did not move \n", objectsThatDidNotMove.name(rowIdx));
end