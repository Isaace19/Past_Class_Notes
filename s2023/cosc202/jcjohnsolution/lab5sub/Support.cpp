...
void ppm::read(const string &in_file)
{
    ...
    if (!file)
    {
        cerr << "ppm::read: cannot open " << in_file << '\n';
        exit(1);
    }
    ...
    if (magic != magicid)
    {
        cerr << "ppm::read: magicid " << magic << " not supported\n";
        exit(1);
    }

    if (max_val != maxvalue)
    {
        cerr << "ppm::read: maxvalue " << max_val << " not supported\n";
        exit(1);
    }
    ...
    if (cur - (char *)img != width * height * sizeof(RGB))
    {
        cerr << "ppm::read: expected " << width * height * sizeof(RGB) << " but read " << (cur - (char *)img) << " bytes\n";
        exit(1);
    }
